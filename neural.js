...
class Layer {
    constructor(
        ...
        for (let i = 0; i < numOutputs; i++) {
            ...
            for (let j = 0; j < numInputs; j++) {
                this.weights[i].push(
                    weightInitFunction(this.numInputs, this.numOutputs)
                );
    ...
    regularizationCost() {
        ...
        return cost / (this.numInputs * this.numOutputs);
    }
    gradientMagnitude() {
        let sum = 0;
        for (let i = 0; i < this.numOutputs; i++) {
...
class NeuralNetwork {
    ...
    costOfAll(dataPoints) {
        ...
        for (let i = 0; i < this.layers.length; i++) {
            cost +=
                this.layers[i].regularizationCost() * this.regularizationRate;
        }
...
const randomWeight = (numInputs, numOutputs) =>
    whileNot(Math.random, 0.5, []) - 0.5; // -0.5 to 0.5
// TODO: ARE THESE RIGHT??
const xavierWeight = (numInputs, numOutputs) =>
    whileNot(gaussianRandom, 0, [0, Math.sqrt(2 / (numInputs + numOutputs))]);
const heWeight = (numInputs, numOutputs) =>
    whileNot(gaussianRandom, 0, [0, Math.sqrt(2 / numInputs)]);
